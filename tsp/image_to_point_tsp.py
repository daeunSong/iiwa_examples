...
def solve_tsp(dat_file, time_bound=300, verbose=True, norm="EUC_2D", seed=42):
...
def plot_tsp(tour_data, cities, color="black", filename="anne", output_prefix='output_', display=True, save=True):
    ...
    for s in lines:
        for (x, y) in s:
            h+= str(x)+ " "+ str(y)+'\n'
    with open('anne.txt', 'w') as the_file:
        the_file.write(h)
...
if __name__ == "__main__":
    ...
    parser.add_argument("filename", metavar=".dat/.npy filename",default='anne',
    ...
    plot_tsp(tour_data, cities, color="blue", filename=args.output_filename,
